...
| `loop-each`              | 断定     | ループ／反復コールバック内、またはローカル assert ヘルパーの反復呼び出しで検証＝手書きパラメータ化。`test.each` に書き換えよ。 |
...
同型の手書きパラメータ化として、**テスト内でローカル定義した「`expect` を含む関数」を 2 回以上呼び出す**形（`const check = (v) => expect(...); check(1); check(2);`）も `loop-each` として扱う。ローカル定義なら静的に検出できるためである。一方、**クロスファイル／外部から import したヘルパー越し**の隠れ expect の個数推定は依然として静的解析の射程外であり行わない（lint の原理的盲点）。`loop-each` の対象はあくまで「同一テスト内で観測可能な」ループ・反復・ローカルヘルパー反復に限る。
...
```
0. ループ／反復コールバック内に expect があれば、またはローカル assert ヘルパーを 2 回以上呼んでいれば → loop-each（direct 個数の判定より先に行う）。
...
- `ng-helper-repeated.ts`: expect 入りローカルヘルパーを 2 回呼ぶ。
...
- `ok-helper-single.ts`: expect 入りローカルヘルパーを 1 回だけ呼ぶ（＝単一検証）。
