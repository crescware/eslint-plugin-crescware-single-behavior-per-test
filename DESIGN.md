...
分類・報告の母集団は **direct expect** に限定する。direct expect とは、テストコールバック本体の `BlockStatement` 直下の `ExpressionStatement` に書かれた `expect` 連鎖である。ループ・if 分岐・コールバック内・ヘルパー関数内にある expect（nested / hidden）は、1 回のテスト実行で何個走るか静的に確定しないため、分類の材料にしない。クロスファイルのヘルパー越しの個数推定は静的解析の射程外であり行わない。
...
- `ok-nested-loop.ts`: ループ内 expect のみ（direct 0）。
- `ok-nested-helper.ts`: ヘルパー関数内 expect のみ。
